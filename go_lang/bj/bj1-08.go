...
func bj10757() {
	...
	for i := bigger; i > 0; i-- {
		numb, _ := strconv.ParseInt(string(b[i-1]), 0, 8)
		numa, _ := strconv.ParseInt(string(a[i-1]), 0, 8)
		sumValue := int(numb + numa)
		if sumValue > 9 {
			answer = fmt.Sprint((sumValue+overflow)%10) + answer
			overflow = (sumValue + (overflow)) / 10
		} else {
			answer = fmt.Sprint((sumValue+overflow)%10) + answer
			overflow = (sumValue + overflow) / 10
		}
	...
	if overflow > 0 {
		answer = fmt.Sprint(overflow) + answer
	}
